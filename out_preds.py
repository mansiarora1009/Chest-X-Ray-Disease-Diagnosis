...
#%%
    ...
    mms_model_test = torch.load("Model_all_20_4-23-18.model")
    #optimizer, criterion, scheduler = init_symbol(mms_model_test)
    #test_loss = valid_epoch(mms_model_test, test_loader, criterion, -1, 'testing')
    out_pred = torch.FloatTensor().cuda()
    out_gt = torch.FloatTensor().cuda()
    for data, target in test_loader:
        ...
        target = Variable(torch.FloatTensor(target).cuda(), volatile=True)
         # Forwards
        output = mms_model_test(data)
        # Log for AUC
        out_pred = torch.cat((out_pred, output.data), 0)
        out_gt = torch.cat((out_gt, target.data), 0)
    np.savetxt("out_preds.csv",out_pred.cpu().numpy(), delimiter=",")
    np.savetxt("out_gt.csv",out_gt.cpu().numpy(), delimiter=",")
    ...
    # #     test_loss_val = valid_epoch(mms_model, test_loader, criterion, j, 'testing')
    #     # LR Schedule
    ...
    #         loss_min = loss_val
    #         torch.save(mms_model, 'Model_all_20_4-23-18.model')
    #         pd.DataFrame(valid_loss).to_csv("valid_loss_all_20_4-23-18.csv")
    #     etime = time.time()
